...
function DocLegalityProgressBar(props) {
  const classes = useStyles();
  let { openingType, status, docLegalityStatus } = props;
  ...
  useEffect(() => {
    ...
    let hasil;
    if (status === 6) {
      if(openingType === "Termin" || openingType === "Replace"){
        hasil = status;
      }else{
        hasil = status - 1;
      }
    ...
    // }
    setIsLoading(false);
  }, [openingType, status, docLegalityStatus]);
...
DocLegalityProgressBar.propTypes = {
  // eslint-disable-next-line react/forbid-prop-types
  dataSteps: PropTypes.array,
  isLoadData: PropTypes.bool,
};

DocLegalityProgressBar.defaultProps = {
  // dataSteps: steps,
  isLoadData: false,
};

export default DocLegalityProgressBar;
