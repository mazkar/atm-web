...
function ImageSelector({selectedImage, onChange, onDelete, title, refId = title, accept="image/*", isLarger = false, largerWidth = 255, largerMinHeight = 165}) {
        ...
        <div className={classesLarger.container}>
          {selectedImage ? (
            <div className={classesLarger.containerImg}>
              <img src={URL.createObjectURL(selectedImage)} width={largerWidth} alt="img"/>
            </div>
          ) : (
            <>
              <input
              ...
              <label htmlFor={refId} className={classesLarger.uploadFile}>
                <div className={classesLarger.containerInput}>
                  <PlusIcon height={24} />
                  <Typography style={{ fontSize: 13, color: "#2B2F3C", fontWeight: 400, textAlign: "center", wordWrap: "break-word" }}>
                  Photo {title}
                  </Typography>
                </div>
              </label>
        ...
        <div className={classes.container}>
          {selectedImage ? (
            <div className={classes.containerImg}>
              <img src={URL.createObjectURL(selectedImage)} width="80" alt="img"/>
            </div>
          ) : (
            <>
              <input
