...
const PopUpRemark = ({ isOpen, onClose, rowToShow, type, rType }) => {
  ...
  useEffect(() => {
    if (isOpen === true) {
      ...
      try {
        ...
        axios
          .post(
            `${process.env.REACT_APP_API_FILE_MANAGEMENT}/getFileRemarkNote`,
            request,
            headers
          )
          .then((res) => {
  ...
  const handleSubmit = () => {
    ...
    try {
      ...
      axios
        .post(
          `${process.env.REACT_APP_API_FILE_MANAGEMENT}/submitFileRemarkNote`,
          request,
          headers
        )
        .then((res) => {
