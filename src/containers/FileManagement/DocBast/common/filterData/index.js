...
import { Paper, Grid, FormControl, Select, MenuItem, ButtonGroup } from "@material-ui/core";
...
import BtnGroupItem from '../../../../Implementation/FilterProgress/BtnGroupItem';
...
const index = (props) => {
  ...
  const [selectedKebutuhan, setSelectedKebutuhan] = useState('need');
  ...
  const onChangeFilter = () => {
    const newDataFilter = {
      ...
      "task":selectedKebutuhan,
    };
    if(newDataFilter.hasOwnProperty('All_Search')){
      delete newDataFilter.All_Search;
    }
    if(newDataFilter.hasOwnProperty('need')){
      delete newDataFilter.need;
    }
    ...
    <Paper className={classes.filterSection}>
      <Grid container justify="space-between" alignItems="center">
              ...
              <div style={{display: "flex"}}>
                <FormControl className={classes.select}>
                  ...
                  >
                    ...
                    {itemSearch.map((item) => {
                      return (
                        <MenuItem value={item.value} onClick={() => setIsFilterActive(true)}>
                          {item.text}
                        </MenuItem>
                      );
                    })}
              ...
              <div style={{display: "flex"}}>
                <FormControl className={classes.select}>
                  <Select
                    ...
                    IconComponent={DropDownIcon}
                  >
                    <MenuItem value="need">Kebutuhan</MenuItem>
                ...
                >
                  {openings.map((val, i) => (
                    <BtnGroupItem
                      ...
                      onClick={() => {onChangeChecklist(i); setOpeningType(val); setIsFilterActive(true);}}
                    >
                      {val}
                    </BtnGroupItem>
                  ))}
        ...
        <Grid item>
          ...
          <MuiButton label="Apply" onClick={()=>{onChangeFilter(selectedSearch);setIsFilterActive(true);}} />
        </Grid>
      </Grid>
    </Paper>
  );
