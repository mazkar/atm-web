...
import {doFetchTicketingClearliness} from "../../../VendorManagement/ApiServices";
...
const TicketingClearliness = () => {
  ...
  const handleOpenDialog = ({ticketId, locationId, mesinId, locationName}) => {
    setDataTicket({ticketId, locationId, mesinId, locationName});
    setOpenModalNewOrder(true);
  };

  function fetchDataTicket() {
    doFetchTicketingClearliness(loadDataHandler, dataRequest).then((response)=>{
      const {content, totalPages: responseTotalPages, totalElements} = response;
      ...
      const newArray = content.map(item => {
        ...
        return {
          ...
          condition: item.conditionBooth,
          question: item.questionSurvey,
