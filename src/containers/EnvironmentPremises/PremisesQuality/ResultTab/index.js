...
import {
  doExportExcelResultPremises,
  doFetchCountPremissesQuality,
  doFetchResultPremisesQuality,
...
const dataDummyResult = [
  {
    ...
    Durasi: "60 menit",
    editAction: (
      <Link to="/vendor-management/digitalisasi/ticketing">Detail</Link>
    ),
  },
  {
    resultId: 13,
...
const ClearlinessResult = () => {
  ...
  const itemSearchTicket = [
    ...
    { text: "Lokasi", value: "location" },
    { text: "Vendor Name", value: "vendorName" },
    { text: "User Vendor", value: "userId" },
    { text: "Category", value: "checklistVendor" },
  ...
  const [dataResult, setDataResult] = useState(dataDummyResult);
  ...
  const handleClickDetail = (id, checklistVendor) => {
    const { url, urlVendor } = dataCard.find(
      (item) => item.type === "premisesQuality"
    );
    history.push(
      isVendor
        ? `${urlVendor}/${id}`
        : `${url}/${id}?typeChecklist=${checklistVendor}`
    );
  };
  ...
  const handleExportExcel = async () => {
    await doExportExcelResultPremises();
  };
  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await doFetchResultPremisesQuality(
        loadDataHandler,
        dataRequest
      );
      const {
      ...
      const formattedArray = content.map((item) => {
        return {
          ...
          userId: item.userId || "-",
          startDate: formatDateReplace(item.startDate) || "-",
          endDate: formatDateReplace(item.endDate) || "-",
          durasi: item.durasi || "-",
          status: item.status || "-",
          action: (
            <Button
              ...
              onClick={() => handleClickDetail(item.id, item.checklistVendor)}
            >
              Detail
            </Button>
          ),
  ...
  useEffect(() => {
    ...
    doFetchCountPremissesQuality(loaderSummary, dataHit)
      .then((response) => {
