...
const FilterComponent = (props) => {
  const { classes, onFilterSubmit, itemSearch, handleReset } = props;
  ...
  const onChangeFilter = () => {
    const newDataFilter = {
      ...
      startDate:
        tglAwal === "" ? "" : moment.unix(tglAwal / 1000).format("YYYY-MM-DD"),
      endDate:
        endDate === "" ? "" : moment.unix(endDate / 1000).format("YYYY-MM-DD"),
    };
  ...
  function disableDate(current) {
    return current && current <= moment(tglAwal);
  }
  return (
