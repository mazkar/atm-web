...
const ModalRejectAll = ({ isOpen, onClose, selectedItems, tabValue, loaderHandler, userId, username }) => {
  ...
  const handleRejectAll = ()=>{
    ...
    function renderDataHit(tab){    
      ...
      if(tab===0){
        ...
        const dataHit={ "idSiteNewAtm": idSiteNewAtm, "locationId":locationId, "userId" : userId,  "rejectRecomendation" : note};
        return dataHit;
      }
      if(tabValue===1){
        try{selectedItems.map((item)=>{
    ...
    if(note === ''){
      ...
      doRejectApproval(loaderHandler, renderDataHit(tabValue)).then(
        respons=>{
      ...
      >
        <Box className={classes.paper}>
          ...
          <Grid container direction="column" style={{marginTop: 10,}}>
            <Grid item>
              <Typography className={classes.title} >{selectedItems.length} {title(tabValue)} ATM Reject</Typography>
            </Grid>
            <Grid item>
              <Typography style={{fontSize: 17, fontWeight: 400, textAlign: 'center'}}>Are you sure want to reject {selectedItems.length} selected {tabValue===0 ? `ID Requester` : `ATM ID`} ?</Typography>
