...
import { doGetDetailNews } from "../../ApiServicesAddOns";
...
const DetailNews = () => {
  ...
  const [detailTitle, setDetailTitle] = useState("news");
  ...
  const getDetailDataHandler = () => {
    doGetDetailNews(loadingHandler, id).then((res) => {
      if (res.responseCode == 200) {
        setDataDetail(res.getDetailNews);
        setDataComment(res.commentNewsDtoList);
        console.log("res comment>>", res.commentNewsDtoList);
      }
    });
