...
import { doGetDetailEvent } from "../../ApiServicesAddOns";
...
const DetailEvent = () => {
  ...
  const [detailTitle, setDetailTitle] = useState("events");
  ...
  const getDetailDataHandler = () => {
    doGetDetailEvent(loadingHandler, id).then((res) => {
      if (res.responseCode == 200) {
        setDataDetail(res.getDetailEvents);
        setDataComment(res.commentEventsDtoList);
        console.log("res comment>>", res.commentEventsDtoList);
      }
    });
