...
function RightComponent({titleCategory, data, onChange}) {
  const classes = useStyles();

  useEffect(() => {
    // eslint-disable-next-line react/prop-types
    // console.log("+++ titleCategory", titleCategory);
    const isNotRequest = titleCategory.toLocaleLowerCase().includes("not request");
    if(isNotRequest){
      onChange("STRIP", "status");
    } else {
          ...
          <Grid item>
            <Grid container direction='column'>
              ...
              <Grid item style={{marginTop: 5}}>
                <SelectWithIcon bordered defaultValue={data.status} suggestions={dataSelectRekeningBank} width='96%' handleChange={(newVal)=> onChange(newVal,"status")} disabled/>
              </Grid>
              <Grid item style={{marginTop: 5}}>
