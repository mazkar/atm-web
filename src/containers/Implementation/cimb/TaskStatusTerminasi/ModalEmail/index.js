...
import { doSendEmailActivation,doGetEmailData} from "../../../ApiServiceImplementation";
...
const ModalEmail = (props) => {
  ...
  const {open, onClose, loadingHandler, onSent,openingType,tipeAtm,dataEmail} = props;
                  ...
                  <Grid container direction="column">
                    ...
                    <Grid container direction="row">
                      <Typography className={classes.txt}>
                        Terlampir jadwal aktivasi <b>{openingType}</b> mesin <b>{tipeAtm}</b> pada hari
                      </Typography>
                      {/*<PickerDate
                        onChange={(e, newVal)=>handleRequest("dateExecution", newVal)}
                        holder="Date Execution"
                    />*/}
                    <DatePicker className="DateSelect" onChange={(e,newVal) => handleRequest("dateExecution",newVal)} clearIcon placeholder="Date Execution" disabledDate={disableDate} suffixIcon={<CalendarIcon/>} allowClear={false} style={{height:34,width:180,marginLeft:5}} getPopupContainer={(triggerNode)=>{
                    ...
                    <Typography className={classes.txt}>
                      pendaftaran mesin <b>{tipeAtm}</b>
                    </Typography>
                    <Typography className={classes.txtBold}>
                      Dan tolong dipersiapkan kunci, kaset dll untuk mesin
                      ATM nya pada saat <b>{openingType}</b>.
                    </Typography>
