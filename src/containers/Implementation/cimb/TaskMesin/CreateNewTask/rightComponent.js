...
function RightComponent(props) {
  ...
  const { leftCategory, data, onChange } = props;
  // const [message, setMessage] = React.useState('')

  useEffect(() => {
    // eslint-disable-next-line react/prop-types
    const isNotRequest = leftCategory.toLocaleLowerCase().includes("not request");
            ...
            <Grid item>
              <Grid container direction='column'>
                ...
                <Grid item style={{marginTop: 5}}>
                  <SelectWithIcon bordered value={data.status} suggestions={dataSelectRekeningBank} width='96%' disabled/>
                </Grid>
                <Grid item style={{marginTop: 5}}>
