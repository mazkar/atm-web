...
import { filterOptionsById } from '../../common/filterStatusOptions';
...
const timeLineData = [
  {userName:'Ghefira Faradhia Shofa', message: 'Change Due Date', initial:'GF', createdDate: 1630296528000, time: '12:36:12 AM'},
  {userName:'Anfika Sigma', message: 'Change Due Date', initial:'AS', createdDate: 1630296528000, time: '12:36:12 AM'},
  {userName:'Tamima ARS', message: 'Change Due Date', initial:'TA', createdDate: 1630296528000, time: '12:36:12 AM'},
  {userName:'Tamima ARS', message: 'Change Due Date', initial:'TA', createdDate: 1630296528000, time: '12:36:12 AM'},
];
...
function RightComponent(props) {
  ...
  const { comments, logHistory, onChangeStatus, onInputComment, onMessageEnter, status } = props;
  ...
  useEffect(() => {
    setStatusOptionToShow(filterOptionsById(dataSelectStatus, status));
  }, [status]);

  React.useEffect(()=>{console.log("Comments: ", comments);},[comments]);
  React.useEffect(()=>{console.log("History: ", logHistory);},[logHistory]);
  React.useEffect(()=>{
    setValue(dataSelectStatus.find(val=>status==val.id)?.value)
  },[status]);
            ...
            <Grid item>
              <Grid container direction='column'>
                ...
                <Grid item style={{marginTop: 5}}>
                  <SelectWithIcon bordered defaultValue={value} suggestions={statusOptionToShow} width='96%' handleChange={handleChange} handleKey={handleChangeKey}/>
                </Grid>
                <Grid item style={{marginTop: 5}}>
