...
function RightComponent(props) {
  ...
  const { data, isLoadData, editValue, onChange } = props;
  ...
  const handleChangeStatus = (dataStatus, keyData) => {
    const items = {
      ...data,
      [keyData]: dataStatus,
    };
  ...
  useEffect(() => {
    try{
      if( data.dataLeftComponent.category === "Not Request" ){
        handleChange("STRIP");
        handleChangeStatus(3, 'status');
      } else {
        handleChange("TODO");
        handleChangeStatus(0, 'status');
      }
    }catch(e){
          ...
          <Grid item>
            <Grid container direction='column'>
              ...
              <Grid item style={{marginTop: 5}}>
                <SelectWithIcon bordered value={value} suggestions={dataSelectRekeningBank} width='96%' handleChange={handleChange} disabled/>
              </Grid>
              <Grid item style={{marginTop: 5}}>
