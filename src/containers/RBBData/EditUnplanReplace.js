...
const EditReplace = () => {
  ...
  useEffect(() => {
    setModalLoader(true);
    axios
      .get(`${process.env.REACT_APP_API_DOMAIN}/masterrbbservices/v1/getUnplanReplaceAtmData/${idRow}`)
      .then((res) => {
        // console.log(res);
        const {
          oldAtmId,
  ...
  const handleSubmit = (e) => {
    ...
    try {
      axios
        .patch(
          `${process.env.REACT_APP_API_DOMAIN}/masterrbbservices/v1/updateUnplanReplaceAtmData/${idRow}`,
          data,
          config
        )
        .then((res) => {
