...
const EditNew = () => {
  ...
  let rowID = "";
  rowID = localStorage.getItem("rowIdEditRenew");
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_API_DOMAIN}/masterrbbservices/v1/getRenewalAtm/${rowID.idRow}`
      )
      .then((res) => {
        // console.log(res);
  ...
  const handleSubmit = (e) => {
    ...
    try {
      axios
        .patch(
          `${process.env.REACT_APP_API_DOMAIN}/masterrbbservices/v1/updateRenewalAtmData/${rowID.idRow}`,
          data,
          config
        )
        .then((res) => {
          ...
          // console.log(res);
          if (res.status === 200) {
            window.location.assign("/rbb-data#Renewal");
          }
        })
        .catch((error) => {
          alert(error);
    ...
    <div className={root}>
      <div className={backButton}>
        <MuiIconLabelButton
          ...
          onClick={() => window.location.assign(`/rbb-data#Renewal`)}
          buttonIcon={<ArrowLeft />}
        />
      </div>

      <Typography className={title}>Edit RBB Renewal</Typography>
      <div>
        {isOpenModalLoader ? (
