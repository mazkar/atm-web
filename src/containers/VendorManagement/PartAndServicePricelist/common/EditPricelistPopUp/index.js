...
const EditPricelistPopUp = ({isOpen = false, onClose, currentData, refresh, vendorId, currentTabs}) => {
  ...
  const fixFormatNumeral = (number) => {
    const fixedNumber = number.split(".").join("")
    return fixedNumber
  }

  const deleteRp = (number) => {
    const splitNumber = number.split(" ")
    if(splitNumber[1]) return fixFormatNumeral(splitNumber[1])
    else return fixFormatNumeral(splitNumber[0])
  }
  ...
  const handleEditPart = async () => {
    ...
    const newPayload = {
      ...
      namaBarang,
      merek,
      hargaBarang: deleteRp(hargaBarang),
      hargaJasaPemasangan: deleteRp(hargaJasaPemasangan)
    };
            ...
            <Grid item xs={6}>
              <div style={{ marginBottom: "14px" }}>
                <LabelTextField
                  ...
                  value={numeral(payload.hargaBarang).format("0,0")}
                />
              </div>
            </Grid>
            <Grid item xs={6}>
              <div style={{ marginBottom: "14px" }}>
                <LabelTextField
                  ...
                  value={numeral(payload.hargaJasaPemasangan).format("0,0")}
                />
              </div>
            </Grid>
          </>
              ...
              <Grid item xs={6}>
                <div style={{ marginBottom: "14px" }}>
                  <LabelTextField
                    ...
                    value={numeral(payload.biayaJasa).format("0,0")}
                  />
                </div>
              </Grid>
            </>
