...
const EditItem = ({ isOpen, onClose, onSuccessSubmit, categoriesId,itemId }) => {
  const {
    modal,
    paper,
    primaryButton,
  ...
  //const {id,name,conditionName} = itemId || {}
  
  const [typeNames, setTypeNames] = useState(itemId.listNameType);
  const [inputTypeNames, setInputTypeNames] = useState("");
  const [existingNames, setExistingNames] = useState(itemId.listExistType);
  const [inputExistingNames, setInputExistingNames] = useState("");
  const [conditionNames, setConditionNames] = useState(itemId.listConditionName);
  const [inputConditionNames, setInputConditionNames] = useState("");
  const [checkTrue, setCheckTrue] = useState(false);
  const [checkExist, setCheckExist] = useState(itemId.listExisting);
  const [checkType, setCheckType] = useState(itemId.listType);
  const [checkCondition, setCheckCondition] = useState(itemId.listCondition);
  ...
  //edit
  const [textName, setTextName] = useState(itemId.name);
  const [photoChek,setPhotoCek]= useState(itemId.photo)

  useEffect(()=>{
      console.log("ini idnnya",itemId) 
  },[itemId])
  function handleChangeName(e) {
    setTextName(e.target.value);
  }
  function handlePhotoChek(e){
    setPhotoCek(e.target.checked)
  }
  ...
  const handleDeleteChipExisting = (chipToDelete) => {
    setExistingNames((chips) => chips.filter((chip) => chip !== chipToDelete));
  };
  ...
  const handleSubmit = async () => {
    //let promises =[];
    //onst types = typeNames.filter((val) => val);
    ...
    const dataHit = {
      ...
      photoStatus: photoChek,
      customizeId: categoriesId,
      ...(typeInput && { inputTypeStatus: true }),
      ...(!typeInput && { typeName: typeNames }),
      ...(exist && { inputExistingStatus: true }),
      ...(!exist && { existingName: existingNames }),
      ...(conditionInput && { inputConditionStatus: true }),
      ...(!conditionInput && { conditionName: conditionNames }),
    };
          ...
          >
            ...
            <Grid item xs={6}>
              <Grid container direction="column" spacing={1}>
                ...
                {existingNames.length == 0 || checkExist == true ? (
                  <div></div>
                ) : (
                  <Grid item>
          ...
          >
            <Grid item xs={6}>
              <Grid container direction="column" spacing={1}>
                ...
                {typeNames.length == 0 || checkType == true ? (
                  <div></div>
                ) : (
                  <Grid item>
                    <Grid container spacing={1}>
            ...
            <Grid item xs={6}>
              <Grid container direction="column" spacing={1}>
                ...
                {conditionNames.length == 0 || checkCondition == true ? (
                  <div></div>
                ) : (
                  <Grid item>
                    <Grid container spacing={1}>
