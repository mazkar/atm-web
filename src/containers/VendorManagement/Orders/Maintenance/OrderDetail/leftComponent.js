...
import { Paper, Typography, Grid, Box, Button, IconButton } from '@material-ui/core';
...
function LeftComponent(props) {
  ...
  const { content, onChange, fetchData } = props;
  ...
  async function handleSave() {
    ...
    try {
      ...
      try {
        setIsOpenModalLoader(true);
        await axios.post(
          `${constants.IMPLEMENTATION_SERVICE}/saveOrUpdateTaskMaintenanceVendor`,
          // console.log(
          {
            invoiceNumber: content.invoiceNumber,
            invoiceFile: content.invoiceFile || path || '',
            id: id * 1,
  ...
  const fieldInformasiCIMBLeft = [
    ...
    { label: 'ID Location', value: content.idLocation },
    {
      label: 'Nama Lokasi',
  ...
  const fieldInformasiCIMBRight = [
    ...
    { label: 'Dimensi', value: content.dimensi },
    ...
    { label: 'Tgl Approved', value: content.approvalDate },
    {
      label: 'Tgl Pengerjaan',
      value: content.processingDate,
    },
    {
  ...
  const fieldInformasiVendorLeft = [
    ...
    { label: 'Tgl Kirim Invoice', value: content.invoiceDate },
  ];
  ...
  const fieldInformasiUmumRight = [
    ...
    { label: 'Media Promosi', value: JSON.parse(content.mediaPromotion)?.toString() },
  ...
  const imagesSebelum = [
    { src: content.photoFront },
    { src: content.photoRight },
    { src: content.photoLeft },
    { src: content.photoRear },
  ];

  const cimbAttachment = content?.cimbAttachment || [];
...
function renderValue(type, value, color, fontWeight, isDone, isAttachment, config) {
            ...
            <Link 
              onClick={()=>{
                if(userRoleName?.toLowerCase().includes('vendor')){
                  history.push(`/vendor-orders/maintenance/bast-digital/${value}`);
                }else{
                  history.push(`/vendor-management/orders/maintenance/bast-digital-preview/${value}`);
                }
              }
              }
  ...
  return (
    <Typography
      style={{ fontWeight: fontWeight || 600, color: color || '#2B2F3C', overflowWrap: 'break-word' }}
    >
      {value}
    </Typography>
  );
