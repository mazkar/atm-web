  ...
  async function handleSave() {
    ...
    try {
      ...
      try {
        setIsOpenModalLoader(true);
        await axios.post(
          `${constants.IMPLEMENTATION_SERVICE}/saveOrUpdateTaskSignageVendor`,
          // console.log(
          {
            ...
            <Link 
              onClick={()=>{
                if(userRoleName?.toLowerCase().includes('vendor')){
                  history.push(`/vendor-orders/signage/bast-digital/${value}`);
                }else{
                  history.push(`/vendor-management/orders/signage/bast-digital-preview/${value}`);
                }
              }
              }
