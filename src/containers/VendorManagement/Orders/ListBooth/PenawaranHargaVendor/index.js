...
import TopComponent from "./paperTop";
import BottomComponent from "./paperBottom";
...
const createKebutuhan = () => {
  ...
  const [typeSet, setType] = useState("booth");
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        console.log("DATA YOKKK", res.data);
        const {
  ...
  const saveComment = (e) => {
    // console.log("+++ MEssage enter", e.key);
    if (e.key === "Enter") {
      e.preventDefault();
      const dataHit = {
        message,
