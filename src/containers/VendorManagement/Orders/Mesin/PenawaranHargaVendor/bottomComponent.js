...
const BottomComponent = ({ onSubmit, onFinishUploadHeader, onFinishUploadFooter, onViewSurat, content, setData,onViewNotEditSurat,typeSurat}) => {
  const classes = useStyles();
  const [isModalLoaderOpen, setIsModalLoaderOpen] = useState(false);

  const { biayaBarang = [], biayaService = [], ppn } = content;
  ...
  async function handleFileFooter(e) {
    ...
    try {
      ...
      setIsModalLoaderOpen(false);
      alert('Upload image success.');
      onFinishUploadFooter(resFile.data);
      console.log('~ resFile?.data', resFile?.data);
    } catch (e) {
      console.log('~ e', e);
      setIsModalLoaderOpen(false);
  ...
  const totalHargaBarang = biayaBarang.reduce((prev, cur) => {
    return cur.quantity != null && cur.price != null ? prev + cur.quantity * cur.price : prev;
  }, 0);
          ...
          <Grid item>
            <Box className={classes.boxStyle}>
              ...
              >
                ...
                >
                  <Typography
                    style={{ fontWeight: 600, marginRight: 25, paddingTop: 5 }}
                  >
                    Total Biaya Jasa
                  </Typography>
                  <SmallInput style={{ width: "4%" }} value="Rp" />
                  <SmallInputFocusRed
                    style={{ width: "19%" }}
                ...
                >
                  {/*<Grid item>
                    <input
                      accept='image/*'
                      style={{ display: 'none' }}
                      id='contained-button-file-footer'
                      type='file'
                      onChange={handleFileFooter}
                    />
                    <label htmlFor='contained-button-file-footer'>
                      <Button
                        variant='contained'
                        disableElevation
                        className={classes.secondaryButton}
                        style={{ textTransform: 'capitalize' }}
                        component='span' // biar bisa trigger upload file browser window
                      >
                        Upload Footer Surat
                      </Button>
                    </label>
