...
const createKebutuhan = () => {
  ...
  async function handleSubmit() {
    setModalLoader(true);
    axios
      .post(`${constants.IMPLEMENTATION_SERVICE}/updateStatusPenawaranHargaCimb`, {
        id,
        taskType : "mesin",
        ...(message && {
          comment: [
  ...
  useEffect(() => {
    setModalLoader(true);
    axios
      .post(`${constants.IMPLEMENTATION_SERVICE}/getPenawaranHargaVendor`, { id: id * 1, taskType : "mesin" })
      .then((res) => {
        // console.log('~ res.data', res.data);
        const { penawaranHargaVendorInfo, baseComment, biayaBarang, biayaService } = res.data;
        const { ppn, approvalStatus, countApproval,
          tangalSurat,
          jenisPekerjaan,
          namaVendor,
          noTlpPerusahaan,
        ...
        setDataBottom({ biayaBarang, biayaService, ppn, baseComment, approvalStatus,countApproval});
      })
      .catch((err) => {
        console.log('~ err', err);
        setModalLoader(false);
