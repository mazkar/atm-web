...
const ModalUpload = ({ isOpen, onClose, onCloseAll }) => {
  ...
  const [isLoading, loaderHandler] = useState(true);
  const [optionAtmIds,setOptionAtmIds] = useState(false);
  const [valueId, setValueId] = useState('');
  const [valueATMId, setValueATMId] = useState('');
  ...
  const handleSubmit = async () => {
    try{
      ...
      const result = await axios({
        url: `${process.env.REACT_APP_API_DOMAIN}implementationservice/v1/addAtmMigrasi`,
        method: 'POST',
        data: dataHit,
        headers: configNew,
      });
      ...
      alert(`Error send data ATM add migrasi...! \n${err}`);
      loaderHandler(false);
    }
  };
