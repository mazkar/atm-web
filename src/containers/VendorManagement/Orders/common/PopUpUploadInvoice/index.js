...
import { doUploadExcelInvoice } from '../../../ApiServices';
...
const PopUpUploadInvoice = ({ isOpen, onClose }) => {
  const {
    root,
    modal,
    paper,
  ...
  const handleSubmit = (event) => {
    ...
    doUploadExcelInvoice(loaderHandler, formData)
      .then((response) => {
  ...
  return (
    <div className={root}>
      ...
      <PopupSucces isOpen={isSuccess} onClose={()=>history.go(0)} message="Upload Pembayaran Berhasil Dilakukan!"/>
    </div>
  );
};

PopUpUploadInvoice.propTypes = {
